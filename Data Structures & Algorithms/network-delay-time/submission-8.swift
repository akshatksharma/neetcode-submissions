...
class Solution {
    ...
    func networkDelayTime(_ times: [[Int]], _ n: Int, _ k: Int) -> Int {
        ...
        while let minNode = heap.popMin() {
            guard visitedDistances[minNode.val] == nil else { // the first time we pop we should set it
                continue
            }

            visitedDistances[minNode.val] = minNode.time

            let neighbors = adjList[minNode.val]
            for (neighbor, additionalTime) in neighbors {
                if visitedDistances[neighbor] == nil  {
                    heap.insert(Node(neighbor, minNode.time + additionalTime))
                }
            }
        }

        if visitedDistances.count < n {
            return -1
        } else {
            return visitedDistances.values.max()!
        }
    }
}
