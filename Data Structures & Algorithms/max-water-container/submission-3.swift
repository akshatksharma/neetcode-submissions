class Solution {
    ...
    */
    ...
    func maxArea(_ heights: [Int]) -> Int {
        var maxHeight = 0
        for (outerIdx, outerHeight) in heights.enumerated() {
            for (innerIdx, innerHeight) in heights.enumerated() {
                if innerIdx > outerIdx {
                    maxHeight = max(maxHeight, calcHeight(outerIdx, innerIdx, heights))
                }
            }
        }
