class Solution {
    ...
    implementation
        ...
        maintain a window of size s1.count and count frequencies within it
        add frequency of R char and remove that of L char as you slide
        compare the frequencies with that of the original map
    ...
    */
    ...
    func checkInclusion(_ s1: String, _ s2: String) -> Bool {
        ...
        var L = 0
        for R in 0..<s2.count {
            let rightChar = s2[R], leftChar = s2[L]

            s2Map[rightChar, default: 0] += 1

            let windowSize = R - L + 1
            if windowSize > s1.count {
                s2Map[leftChar, default: 0] -= 1
                if s2Map[leftChar] == 0 {
