class Solution {
    ...
    func canPartitionImpl(_ nums: [Int], _ target: Int, _ index: Int, _ memo: inout [[Bool?]]) -> Bool {
        ...
        // skip
        let canPartitionWithSkip = canPartitionImpl(nums, target, index + 1, &memo)
        // add
        let canPartitionWithoutSkip = canPartitionImpl(nums, target - nums[index], index + 1, &memo)

        let canPartition = canPartitionWithSkip || canPartitionWithoutSkip
